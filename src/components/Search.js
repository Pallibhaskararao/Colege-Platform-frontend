...
const Search = () => {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      try {
        const postsRes = await axios.get('http://https://colege-platform-backend.onrender.com/api/posts/recommendations', {
          headers: { Authorization: `Bearer ${token}` },
        });
        const usersRes = await axios.get('http://https://colege-platform-backend.onrender.com/api/users/recommendations', {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.get(`http://https://colege-platform-backend.onrender.com/api/search?query=${query}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setResults(res.data);
    } catch (err) {
