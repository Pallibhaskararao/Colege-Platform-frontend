...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await axios.get('http://https://colege-platform-backend.onrender.com/api/notifications', {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      console.log('Socket.IO connected, joined room:', userId);
    });
    ...
    socket.on('notificationDeleted', (data) => {
      console.log('Received notificationDeleted event:', data);
      setNotifications((prev) => {
        const updated = prev.filter((n) => n._id.toString() !== data.notificationId.toString());
  ...
  const handleNotificationClick = async (notification) => {
    try {
      if (!notification.read) {
        await axios.put(
          `http://https://colege-platform-backend.onrender.com/api/notifications/${notification._id}/read`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          }
      ...
      if (!notification.viewed) {
        await axios.put(
          `http://https://colege-platform-backend.onrender.com/api/notifications/${notification._id}/viewed`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  const handleMarkAllAsViewed = async () => {
    try {
      await axios.put(
        'http://https://colege-platform-backend.onrender.com/api/notifications/viewed',
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleAcceptRequest = async (requestId, notificationId) => {
    try {
      if (!requestId) {
        throw new Error('Request ID is undefined. This notification may be outdated.');
      }
      console.log('Accepting request with ID:', requestId, 'Notification ID:', notificationId);
      await axios.put(
        `http://https://colege-platform-backend.onrender.com/api/users/requests/${requestId}/accept`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNotifications((prev) => {
        const updated = prev.filter((n) => n._id.toString() !== notificationId.toString());
        console.log('After accepting, updated notifications:', updated);
        return updated;
      });
      // Fallback: Re-fetch notifications to ensure the UI is in sync
      await fetchNotifications();
      alert('Friend request accepted!');
    } catch (err) {
      console.error('Error accepting request:', err);
      alert(err.response?.data?.message || err.message || 'Failed to accept request');
      // Re-fetch on error to ensure the UI is in sync
      await fetchNotifications();
    }
  };
  ...
  const handleDeclineRequest = async (requestId, notificationId) => {
    try {
      ...
      await axios.put(
        `http://https://colege-platform-backend.onrender.com/api/users/requests/${requestId}/decline`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNotifications((prev) => {
        const updated = prev.filter((n) => n._id.toString() !== notificationId.toString());
        console.log('After declining, updated notifications:', updated);
        return updated;
      });
      // Fallback: Re-fetch notifications to ensure the UI is in sync
      await fetchNotifications();
      ...
      alert(err.response?.data?.message || err.message || 'Failed to decline request');
      // Re-fetch on error to ensure the UI is in sync
      await fetchNotifications();
    }
  };
                ...
                <div className="notification-content">
                  ...
                  {notification.type === 'friend_request' && (
                    <div className="friend-request-actions">
                      <button
                        onClick={() =>
                          handleAcceptRequest(notification.requestId, notification._id)
                        }
                      ...
                      <button
                        onClick={() =>
                          handleDeclineRequest(notification.requestId, notification._id)
                        }
