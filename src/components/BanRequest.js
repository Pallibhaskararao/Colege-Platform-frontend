...
const BanRequest = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        if (postId !== 'null') {
          const postResponse = await axios.get(`http://locahost:5000/api/posts/${postId}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          setPost(postResponse.data);
        }

        const userResponse = await axios.get(`http://locahost:5000/api/users/${userId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        'http://locahost:5000/api/ban-requests',
        {
          userToBan: userId,
          reason: reason.trim(),
