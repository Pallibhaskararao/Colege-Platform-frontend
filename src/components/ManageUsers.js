...
const ManageUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get('http://https://colege-platform-backend.onrender.com/api/users', {
    ...
    const fetchBanRequests = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get('http://https://colege-platform-backend.onrender.com/api/ban-requests', {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleBan = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://https://colege-platform-backend.onrender.com/api/users/${userId}/ban`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUnban = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://https://colege-platform-backend.onrender.com/api/users/${userId}/unban`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleApproveBanRequest = async (banRequestId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://https://colege-platform-backend.onrender.com/api/ban-requests/${banRequestId}/approve`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRejectBanRequest = async (banRequestId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://https://colege-platform-backend.onrender.com/api/ban-requests/${banRequestId}/reject`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success(response.data.message);
      setBanRequests(banRequests.filter(request => request._id !== banRequestId));
